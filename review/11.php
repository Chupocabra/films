...
$sql = $dbh->query('SELECT * FROM review WHERE id=11');
$film=$sql->fetch();
$sql = $dbh->query('SELECT * FROM user JOIN review WHERE review.author=user.id');
$author=$sql->fetch();
$sql = $dbh->query('SELECT * FROM comment JOIN review WHERE review.id=comment.id');
$comment = $sql->fetch();
?>
...
<html lang="ru">
    ...
    <body>
        ...
        <main class="main">
            ...
            <div class="comments">
                <?php
                foreach ($dbh->query("SELECT * FROM comment JOIN review WHERE review.id=comment.id") as $row):
                    ?>
                        <ul>
                            <li>
